...
pub async fn run(app: &mut App, socket: &Path) -> Result<()> {
    ...
    let _last_refresh = std::time::Instant::now();
    ...
    loop {
        ...
        // Postgres viewer doesn't auto-refresh — queries fire on
        // explicit Ctrl+Enter. (Ticket-viewer siblings poll every
        // N seconds; that pattern doesn't fit a SQL query editor.)
